...
RSpec.describe 'Merchants' do
  describe 'Merchants Business Intelligence' do
    it 'Can return the top 3 merchants by total items sold' do
      ...
      expect(top_merchants["data"][0]["attributes"]["name"]).to eq(merchant_3.name)
      expect(top_merchants["data"][1]["attributes"]["name"]).to eq(merchant_1.name)
      expect(top_merchants["data"][2]["attributes"]["name"]).to eq(merchant_2.name)
    end
  end
end
