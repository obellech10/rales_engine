...
describe 'Invoice Items Record Endpoints' do
  ...
  it "Can find an invoice item by it's created_at date" do
    invoice_items_4 = @invoice_4.invoice_items.create!(item_id: @item_1.id, quantity: 2, unit_price: @item_1.unit_price, created_at: "2019-06-15 15:38:30 UTC", updated_at: "2019-08-15 15:38:30 UTC")
    invoice_items_5 = @invoice_5.invoice_items.create!(item_id: @item_4.id, quantity: 1, unit_price: @item_4.unit_price, created_at: "2019-07-15 15:38:30 UTC", updated_at: "2019-08-15 15:38:30 UTC")

    get "/api/v1/invoice_items/find?created_at=#{invoice_items_4.created_at}"

    expect(response).to be_successful

    invoice_item = JSON.parse(response.body)

    expect(invoice_item["data"]["attributes"]["id"]).to eq(invoice_items_4.id)
  end

  it "Can find an invoice item by it's updated_at date" do
    invoice_items_4 = @invoice_4.invoice_items.create!(item_id: @item_1.id, quantity: 2, unit_price: @item_1.unit_price, created_at: "2019-06-15 15:38:30 UTC", updated_at: "2019-08-15 15:38:30 UTC")
    invoice_items_5 = @invoice_5.invoice_items.create!(item_id: @item_4.id, quantity: 1, unit_price: @item_4.unit_price, created_at: "2019-07-15 15:38:30 UTC", updated_at: "2019-04-15 15:38:30 UTC")

    get "/api/v1/invoice_items/find?updated_at=#{invoice_items_5.updated_at}"

    expect(response).to be_successful

    invoice_item = JSON.parse(response.body)

    expect(invoice_item["data"]["attributes"]["id"]).to eq(invoice_items_5.id)
  end
end
