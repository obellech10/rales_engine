...
describe 'Invoice Items relationship endpoints' do
  before :each do
    ...
    @invoice_items_3 = @invoice_3.invoice_items.create!(item_id: @item_1.id, quantity: 5, unit_price: @item_1.unit_price, created_at: "2019-08-15 15:38:30 UTC", updated_at: "2019-08-15 15:38:30 UTC")
    @invoice_items_4 = @invoice_4.invoice_items.create!(item_id: @item_2.id, quantity: 2, unit_price: @item_2.unit_price, created_at: "2019-08-15 15:38:30 UTC", updated_at: "2019-08-15 15:38:30 UTC")
  ...
  it "Can return the related invoice for an invoice_item" do
    get "/api/v1/invoice_items/#{@invoice_items_3.id}/invoice"

    invoice = JSON.parse(response.body)

    expect(response).to be_successful
    expect(invoice["data"]["attributes"]["id"]).to eq(@invoice_items_3.invoice_id)
  end

  it "Can return the associated item" do
    get "/api/v1/invoice_items/#{@invoice_items_4.id}/item"

    item = JSON.parse(response.body)

    expect(response).to be_successful
    expect(item["data"]["attributes"]["id"]).to eq(@invoice_items_4.item_id)
  end
end
